...
function AdminDashboard() {
  ...
  const [isHovered, setIsHovered] = useState(false); // FIXED: Added missing state for UI hover effect
  ...
  const fetchTeacherName = async (teacherEmailToSearch) => {
    if (!teacherEmailToSearch) return null;
    try {
  ...
  const handlePgSubmit = async (event) => {
    ...
    try {
      ...
      // FIXED: Using API_BASE_URL for consistency
  ...
  const handleUgSubmit = async (event) => {
    ...
    if (failedRegNos.length > 0) {
      toast.warn(`Failed to find or enroll: ${failedRegNos.join(', ')}.`);
    }
  ...
  // FIXED: Removed the duplicate 'handleExcelUpload' function. This one is used by the button.
  const handleExcelEnroll = async () => {
    ...
    try {
      ...
      for (const row of rows) {
        try {
          // Validate required fields from Excel row
          if (!row.StudentName || !row.RegisterNumber || !row.Email || !row.CourseName || !row.TeacherEmail) {
          ...
          await axios.post(`${API_BASE_URL}/enroll`, {
            ...
            groupRegisterNumbers: row.GroupRegisterNumbers ? String(row.GroupRegisterNumbers).split(",").map(s => s.trim()) : [],
          });
      ...
      setExcelFile(null); // Clear the file input after processing
    } catch (err) {
  ...
  // --- UG Form Dynamic Input Handlers ---
    ...
    <div className='teacher-dashboard-layout'>
      <div className='cont'>
        <div className="admin-sidebar">
          <h2>Admin Panel</h2>
          <button onClick={() => setStudentType('UG')} className={studentType === 'UG' ? 'active' : ''}>
            UG Students
          </button>
          <button onClick={() => setStudentType('PG')} className={studentType === 'PG' ? 'active' : ''}>
            PG Students
          </button>
        </div>
        ...
        <div className="dashboard-content">
          ...
          {studentType === 'PG' && (
            <div>
              ...
              <form onSubmit={handlePgSubmit}>
                ...
                <input id="pgStudentRegNo" type="text" value={pgStudentRegNo} onChange={(e) => setPgStudentRegNo(e.target.value)} placeholder="Enter student's register number" required />
                <label htmlFor="pgProjectName">Project Name:</label>
                <input id="pgProjectName" type="text" value={pgProjectName} onChange={(e) => setPgProjectName(e.target.value)} placeholder="Enter project name" required />
                <label htmlFor="pgTeacherEmail">Teacher Email:</label>
                <input id="pgTeacherEmail" type="email" value={pgTeacherEmail} onChange={(e) => setPgTeacherEmail(e.target.value)} placeholder="Enter teacher's email" required />
          ...
          {studentType === 'UG' && (
            <div>
              ...
              <form onSubmit={handleUgSubmit}>
                ...
                <input id="ugProjectName" type="text" value={ugProjectName} onChange={(e) => setUgProjectName(e.target.value)} placeholder="Enter project name" required />
                {ugStudentRegNos.map((regNo, index) => (
                  <div key={index}>
                    <label htmlFor={`ugStudentRegNo${index}`}>Student Register Number {index + 1}:</label>
                    <input id={`ugStudentRegNo${index}`} type="text" value={regNo} onChange={(e) => handleUgRegNoChange(index, e.target.value)} placeholder="Enter student's register number" />
                  </div>
                ))}
                ...
                <input id="ugTeacherEmail" type="email" value={ugTeacherEmail} onChange={(e) => setUgTeacherEmail(e.target.value)} placeholder="Enter teacher's email" required />
          ...
          {/* FIXED: Bulk Import Section is now properly integrated */}
          <div className="excel-upload" style={{ marginTop: "30px" }}>
            <h3 style={{ marginBottom: '10px' }}>Bulk Import from Excel</h3>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: '#f9f9f9', padding: '15px', border: '1px solid #ddd', borderRadius: '8px', maxWidth: '500px' }}>
              <input id="excel-file-upload" type="file" accept=".xlsx, .xls" onChange={(e) => setExcelFile(e.target.files[0])} style={{ display: 'none' }} />
              <label htmlFor="excel-file-upload" style={{ padding: '6px 12px', borderRadius: '6px', backgroundColor: '#007BFF', color: 'white', cursor: 'pointer', border: 'none', whiteSpace: 'nowrap' }}>
                Select File
              </label>
              <span style={{ flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: '#555' }}>
                {excelFile ? excelFile.name : "No file chosen"}
              </span>
              <button onClick={handleExcelEnroll} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', backgroundColor: isHovered ? '#45a049' : '#4caf50', color: 'white', cursor: 'pointer', transition: 'background-color 0.2s' }}>
                Enroll
              </button>
            </div>
          </div>
